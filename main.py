...
def get_working_sheets():
    try:
        ...
        # favorites 시트는 더 이상 직접 쓰지 않고 유저 확인용으로만 유지하거나 무시합니다.
...
# --------------------------------------------------
# 5. 즐겨찾기 연동 (핵심 수정 부분: 시트의 '별표' 열 참조)
# --------------------------------------------------
# 이제 별도의 fav_sheet를 조회하지 않고, 로드된 df의 '별표' 열을 사용합니다.
if "별표" in df.columns:
    # '별표' 열에 값이 비어있지 않은(공백이 아닌) 행의 PK들을 즐겨찾기 세트로 저장
